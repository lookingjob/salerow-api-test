...
# cron
#COPY --link docker/php/crontasks /var/spool/cron/crontabs/root
#COPY --link docker/php/entrypoint.bash /usr/sbin
#RUN chmod a+x /usr/sbin/entrypoint.bash
#ENTRYPOINT /usr/sbin/entrypoint.bash
RUN echo '*/5  *  *  *  *    php /srv/app/bin/console app:create-indexes --marketCap=50000000' > /etc/crontabs/root
#RUN echo '*/15  *  *  *  *    php /srv/app/bin/console app:create-indexes --marketCap=50000000' > /etc/periodic/15min/root
#RUN chmod -R a+x /etc/periodic/15min
#RUN crond -b
RUN crond -b -l 2 -L /dev/stdout
#CMD ['crond', '-l 2', '-b', '-L /dev/stdout']
